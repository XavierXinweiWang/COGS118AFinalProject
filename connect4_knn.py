        ...
        for k in range(foldNum):
            ...
            if k == 0:
                ...
                if k == foldNum - 1:
                    ...
                    # print k
                    trainStack = np.vstack((trainingSet[0:k * (len(trainingSet) / foldNum), 0:43],
                                            trainingSet[(k + 1) * (len(trainingSet) / foldNum):, 0:43]))
                    validationStack = trainingSet[
                                      k * (len(trainingSet) / foldNum):(k + 1) * (len(trainingSet) / foldNum), 0:43]
                    # print 'Train set: ' + repr(len(trainStack))
                    # print 'Test set: ' + repr(len(validationStack))
